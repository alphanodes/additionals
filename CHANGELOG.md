...
- An avatar rendered next to its name (`user_with_avatar`, class `additionals-avatar`) keeps its inline spacing on a detail page. Redmine floats every avatar below `div.issue` and gives it the 12px margin of the large author avatar it draws there, and the detail pages of our plugins (DB entries, passwords, budgets, AI prompts, templates, wiki pages, contacts, invoices) carry that class only to inherit core's detail page styling - the avatar was pulled out of its line and left a visible gap in front of the name
